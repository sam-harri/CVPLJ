    ...
    for s in subset:
        ...
        for idx, p in enumerate(s[:-2]):  # Include the index of the point in the array
            if p >= 0:  # Ensure the index is valid
                c = candidate[int(p)]
                new_person.append((c[0], c[1], idx))  # Append the x, y coordinates along with the point index
            else:
...
def draw_colored_bodypose(canvas, people):
    ...
    colors = [(int(c[0]*255), int(c[1]*255), int(c[2]*255)) for c in colors]  # Convert from RGB to BGR for OpenCV

    for person_idx, person in enumerate(people):
        for _, keypoint in enumerate(person):
            if keypoint:
                x, y, _ = keypoint
                cv2.circle(canvas, (int(x), int(y)), 4, colors[person_idx], thickness=-1)

        for _, (start_idx, end_idx) in enumerate(limbSeq):
            start_point = person[start_idx] if start_idx < len(person) and person[start_idx] else None
            end_point = person[end_idx] if end_idx < len(person) and person[end_idx] else None
